from math import sqrt, cbrt, pow, ceil, floor, fabs
import math
import discord
import asyncio
...
class CountingCog(commands.Cog):
    def __init__(self, client):
        ...
        self.evaluator = simpleeval.SimpleEval(
            ...
            names={'e': math.e},
            operators={
                o: simpleeval.DEFAULT_OPERATORS[o] for o in [
                    ast.Add,
                    ast.Sub,
                    ast.Mult,
                    ast.Div,
    ...
    def get_lowest_score_channel_id_from_scores(self, scores):
        ...
        for channelid in scores:
            if scores[channelid][
                "score"] <= lowestscore:  # using <= on purpose here so we're taking out the bottom of the leaderboard even if we have a tie
    ...
    async def on_message(self, message):
        ...
        if message.content.startswith("w|"):
            ...
            if settings["EnableBinary"]:
                digits = "01"
                try:
                    ex = self.parse_and_convert_binary(message.content.split('|')[1])
            ...
            if settings["EnableExpressions"]:
                ...
                except (simpleeval.InvalidExpression, KeyError, SyntaxError, ValueError) as e:
                    #logger.error(e)
                    await message.add_reaction("⁉")
                except Exception as e:
                    #pass
    ...
    async def attempt_count(self, message, guess):
        logger.debug('attempting to count')
        if not self.is_channel_registered(message.channel.id): return
        settings = self.get_channel_settings(message.channel.id)
        goal_number, previous_author, timescounted = self.get_channel_data(message.channel.id,
                                                                           settings["ForceIntegerConversions"])
        ...
        if message.author.id == previous_author and not settings["AllowSingleUserCount"]:
            ...
            await message.reply(
                f"Oof, you failed! You counted twice in a row. If you feel this was unjustified, contact the mods. The next number is {nextnumber}.")
            died = True
            ...
            nextnumber = settings["StartingNumber"] + settings["Step"]
            await message.reply(
                f"Oof, you failed! The next number was {goal_number}, but you said {guess}. If you feel this was unjustified, contact the mods. The next number is {nextnumber}.")
            died = True
        if died:
            await message.add_reaction("⚠")
            if timescounted > highscore:
    ...
    def parse_and_convert_binary(self, input):
        result = int(input, 2)
        return result
    ...
    @bridge.bridge_command(name='config')
...
EnableBinary - Whether to enable counting in binary (defaults to true)
...
def setup(client):
    client.add_cog(CountingCog(client))
